...
/**
 * 
 * Classe qui gère le feu. Le feu peut bruler ses voisins, bruler la case ou il
 * est ou se faire éteindre par les forces de combat du feu.
 * 
 * @author Nicolas
 * 
 */
public class Feu extends Acteur {
	...
	public void agi(Carte maCarte) {
		...
		if (!maCarte.getTabHexagones(X, Y).isInflammable()) {
			...
			System.out
					.println("Cette case n'est pas inflammable. Essayez encore.");
			return;
		}
		...
		if (apparition) {
			...
			// On baisse l'humidité d'un montant important au début.
			...
			for (int z = 0; z < 6; z++) {
				if (feuVoisins.get(z) == true) {
					if (allumeeeeeerLeFeuuuu) {
						...
						// On met le feu au voisin courant.
						if (yMettreLeFeu.x >= 0
								&& yMettreLeFeu.y >= 0
	...
	/**
	 * Fonction déterminant si un feu se propage à ses voisins. Elle correspond
	 * à une grande partie de l'équilibrage du programme. Une carte est
	 * nécessaire pour récupérer force et direction du vent.
	 * 
			...
			if (mesVoisinsPoint.get(i).x >= 0) { // Si on est bien dans la map.
				if (mesVoisins.get(i).isInflammable()) {
					...
					if (humidite >= 100) {
						...
						// Humidité trop élevée, impossibilité de bruler.
					}

					double proba = probaOU((1.0 / humidite),
							(transmission / 14));
	...
	private static ArrayList<Double> ventProba(Carte maCarte) {
		...
		// La direction du vent global de la carte converti en entier.
